...
public class ImGuiRenderer
{
    ...
    public virtual void BeforeLayout(GameTime gameTime)
    {
        ...
        if (dt <= 0f)
            dt = 1f / 60f; // Default to 60 FPS if zero or negative
    ...
    private unsafe void UpdateBuffers(ImDrawDataPtr drawData)
    {
        ...
        if (_indexData is null || _vertexData is null)
        {
            throw new InvalidOperationException("Buffers were not properly initialized");
        }
        if (_indexBuffer is null || _vertexBuffer is null)
        {
            throw new InvalidOperationException("Buffers were not properly initialized");
        }
